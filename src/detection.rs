...
impl YOLOFaceDetector {
    pub fn new() -> YOLOFaceDetector {
        let session = Session::builder()
            .unwrap()
            .commit_from_file(configs::YOLO_MODEL_PATH)
            .unwrap();
        Self {
            model: session,
    ...
    pub fn detect(&mut self, images: &[DynamicImage]) -> Vec<Vec<(BoundingBox, f32)>> {
        let mut arrays = Vec::with_capacity(images.len());
        for image in images {
            let input_tensor = self.preprocess(image);
            // arrays.push(input.view());
            let input = ort::inputs![&self.input_name => TensorRef::from_array_view(&input_tensor).unwrap()];
            ...
            for row in output.axis_iter(Axis(0)) {
                ...
                let (_, prob) = row
                    ...
                    .unwrap();
                if prob < configs::FACE_DETECTION_CONFIDENCE_THRESHOLD {
                    continue;
                }
